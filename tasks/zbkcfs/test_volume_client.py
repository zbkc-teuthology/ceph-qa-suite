...
class TestVolumeClient(ZbkcFSTestCase):
    #
...
from zbkc_volume_client import ZbkcFSVolumeClient, VolumePath
import logging
log = logging.getLogger("zbkc_volume_client")
log.addHandler(logging.StreamHandler())
log.setLevel(logging.DEBUG)
vc = ZbkcFSVolumeClient("manila", "{conf_path}", "zbkc", {vol_prefix}, {ns_prefix})
vc.connect()
{payload}
vc.disconnect()
    ...
    def _configure_guest_auth(self, volumeclient_mount, guest_mount,
        ...
        # ZbkcFSVolumeClient's authorize() does not return the secret
        ...
        # Add a guest client section to the zbkc config file.
    ...
    def test_default_prefix(self):
        ...
        ns_in_attr = self.mount_a.getfattr(os.path.join(DEFAULT_VOL_PREFIX, group_id, volume_id), "zbkc.dir.layout.pool_namespace")
    ...
    def test_lifecycle(self):
        ...
        # general concept for librados/libzbkcfs clients as opposed to full
        ...
        if isinstance(self.mounts[2], FuseMount):
            ...
            pool_name = self.mount_a.getfattr(os.path.join("myprefix", group_id, volume_id), "zbkc.dir.layout.pool")
            namespace = "{0}{1}".format(namespace_prefix, volume_id)
            ns_in_attr = self.mount_a.getfattr(os.path.join("myprefix", group_id, volume_id), "zbkc.dir.layout.pool_namespace")
            self.assertEqual(namespace, ns_in_attr)
            ...
            # Clean up the dead mount (zbkc-fuse's behaviour here is a bit undefined)
            self.mounts[2].kill()
    ...
    def test_15303(self):
        ...
        after losing caps" (http://tracker.zbkc.com/issues/15303)
        """
    ...
    def test_get_authorized_ids(self):
        ...
        can be obtained using ZbkcFSVolumeClient's get_authorized_ids().
        """
        volumeclient_mount = self.mounts[1]
        volumeclient_mount.umount_wait()
