...
class ZbkcFSMount(object):
    ...
    def umount_wait(self, force=False, require_clean=False):
        ...
        :param force: Expect that the mount will not shutdown cleanly: kill
                      it hard.
        :param require_clean: Wait for the Zbkc client associated with the
                              mount (e.g. zbkc-fuse) to terminate, and
    ...
    def get_keyring_path(self):
        return '/etc/zbkc/zbkc.client.{id}.keyring'.format(id=self.client_id)

    @property
    def config_path(self):
        """
        Path to zbkc.conf: override this if you're not a normal systemwide zbkc install
        :return: stringv
        """
        return "/etc/zbkc/zbkc.conf"
    ...
    def open_n_background(self, fs_path, count):
        ...
        :param fs_path: Path relative to ZbkcFS root, e.g. "foo/bar"
