...
class NonDefaultLayout(Workload):
    ...
    def write(self):
        ...
        self._mount.run_shell(["setfattr", "-n", "zbkc.file.layout.object_size", "-v", "8388608", "./datafile"])
        self._mount.run_shell(["dd", "if=/dev/urandom", "of=./datafile", "bs=1M", "count=32"])
        self._initial_state = self._mount.stat("datafile")

    def validate(self):
        p = self._mount.run_shell(["getfattr", "--only-values", "-n", "zbkc.file.layout.object_size", "./datafile"])
...
class TestDataScan(ZbkcFSTestCase):
    MDSS_REQUIRED = 2
