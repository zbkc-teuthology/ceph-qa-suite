...
def needs_trimming(f):
    """
    Mark fn as requiring a client capable of trimming its cache (i.e. for zbkc-fuse
...
class ZbkcFSTestCase(ZbkcTestCase):
    """
    Test case for Zbkc FS, requires caller to populate Filesystem and Mounts,
    ...
    def setUp(self):
        super(ZbkcFSTestCase, self).setUp()
        ...
        try:
            ...
            # Fallback for older Zbkc cluster
    ...
    def tearDown(self):
        super(ZbkcFSTestCase, self).tearDown()
        ...
        for subsys, key in self.configs_set:
            self.mds_cluster.clear_zbkc_conf(subsys, key)

    def set_conf(self, subsys, key, value):
        self.configs_set.add((subsys, key))
        self.mds_cluster.set_zbkc_conf(subsys, key, value)

    def auth_list(self):
        """
        Convenience wrapper on "zbkc auth list"
        """
