...
from .mount import ZbkcFSMount

log = logging.getLogger(__name__)


class FuseMount(ZbkcFSMount):
    ...
    def mount(self, mount_path=None, mount_fs_name=None):
        ...
        log.info('Mounting zbkc-fuse client.{id} at {remote} {mnt}...'.format(
        ...
        fuse_cmd += [
            ...
            # TODO zbkc-fuse doesn't understand dash dash '--',
            self.mountpoint,
        ]
        ...
        proc = self.client_remote.run(
            args=run_cmd,
            logger=log.getChild('zbkc-fuse.{id}'.format(id=self.client_id)),
            stdin=run.PIPE,
            wait=False,
        )
    ...
    def is_mounted(self):
        ...
        if fstype == 'fuseblk':
            log.info('zbkc-fuse is mounted on %s', self.mountpoint)
            return True
        else:
            log.debug('zbkc-fuse not mounted, got fs type {fstype!r}'.format(
                fstype=fstype))
            return False
    ...
    def umount(self):
        try:
            ...
            log.info('Failed to unmount zbkc-fuse on {name}, aborting...'.format(name=self.client_remote.name))
    ...
    def umount_wait(self, force=False, require_clean=False):
        ...
        if force:
            ...
            # When we expect to be forcing, kill the zbkc-fuse process directly.
        ...
        try:
            ...
            log.error("process failed to terminate after unmount.  This probably"
                      "indicates a bug within zbkc-fuse.")
            raise
        except CommandFailedError:
            if require_clean:
                raise
    ...
    def _asok_path(self):
        return "/var/run/zbkc/zbkc-client.{0}.*.asok".format(self.client_id)
...
""".format(
        ...
        p = self.client_remote.run(
            args=['sudo', self._prefix + 'zbkc', '--admin-daemon', asok_path] + args,
            stdout=StringIO())
        return json.loads(p.stdout.getvalue())

    def get_global_id(self):
        """
        Look up the ZbkcFS client ID for this mount
        """
