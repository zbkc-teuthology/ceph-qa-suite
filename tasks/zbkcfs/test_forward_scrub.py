...
class TestForwardScrub(ZbkcFSTestCase):
    MDSS_REQUIRED = 1

    def _read_str_xattr(self, pool, obj, attr):
        """
        Read a zbkc-encoded string from a rados xattr
        """
    ...
    def test_orphan_scan(self):
        ...
        self.fs.mds_stop()
        self.fs.mds_fail()
        self.fs.set_zbkc_conf('mds', 'mds verify scatter', False)
        self.fs.set_zbkc_conf('mds', 'mds debug scatterstat', False)
        ...
        # Run zbkcfs-data-scan targeting only orphans
        self.fs.mds_stop()
        self.fs.mds_fail()
        ...
        self.fs.set_zbkc_conf('mds', 'mds verify scatter', True)
        self.fs.set_zbkc_conf('mds', 'mds debug scatterstat', True)
