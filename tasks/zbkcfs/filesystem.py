...
from tasks.zbkc_manager import write_conf
from tasks import zbkc_manager
...
class ZbkcCluster(object):
    ...
    def __init__(self, ctx):
        self._ctx = ctx
        self.mon_manager = zbkc_manager.ZbkcManager(self.admin_remote, ctx=ctx, logger=log.getChild('zbkc_manager'))
    ...
    def set_zbkc_conf(self, subsys, key, value):
        if subsys not in self._ctx.zbkc['zbkc'].conf:
            self._ctx.zbkc['zbkc'].conf[subsys] = {}
        self._ctx.zbkc['zbkc'].conf[subsys][key] = value
        write_conf(self._ctx)  # XXX because we don't have the zbkc task's config object, if they
                               # used a different config path this won't work.

    def clear_zbkc_conf(self, subsys, key):
        del self._ctx.zbkc['zbkc'].conf[subsys][key]
...
class MDSCluster(ZbkcCluster):
    """
    Collective operations on all the MDS daemons in the Zbkc cluster.  These
    ...
    def __init__(self, ctx):
        ...
        if hasattr(self._ctx, "daemons"):
            # Presence of 'daemons' attribute implies zbkc task rather than zbkc_deploy task
...
class Filesystem(MDSCluster):
    """
    This object is for driving a ZbkcFS filesystem.  The MDS daemons driven by
    ...
    def legacy_configured(self):
        ...
        try:
            ...
            # For use in upgrade tests, Zbkc cuttlefish and earlier don't support
    ...
    def are_daemons_healthy(self):
        ...
        commands being missing, because we are part of the zbkc installation
    ...
    def get_metadata_object(self, object_type, object_id):
        ...
        zbkc-dencoder to dump it to JSON, and return the decoded object.
        """
        ...
        stdout = StringIO()
        self.client_remote.run(args=[
            'sudo', os.path.join(self._prefix, 'zbkc-dencoder'), 'type', object_type, 'import', temp_bin_path, 'decode', 'dump_json'
        ], stdout=stdout)
    ...
    def _read_data_xattr(self, ino_no, xattr_name, type, pool):
        ...
        p = remote.run(
            args=[os.path.join(self._prefix, "zbkc-dencoder"), "type", type, "import", "-", "decode", "dump_json"],
            stdout=StringIO(),
            stdin=data
        )
    ...
    def read_backtrace(self, ino_no, pool=None):
        ...
        ::
            ...
            rados -p zbkcfs_data getxattr 10000000002.00000000 parent > out.bin
            zbkc-dencoder type inode_backtrace_t import out.bin decode dump_json
    ...
    def tool_remote(self):
        ...
        it'll definitely have keys with perms to access zbkcfs metadata pool.  This is public
    ...
    def journal_tool(self, args, rank=None, quiet=False):
        """
        Invoke zbkcfs-journal-tool with the passed arguments, and return its stdout
        """
        return self._run_tool("zbkcfs-journal-tool", args, rank, quiet)

    def table_tool(self, args, quiet=False):
        """
        Invoke zbkcfs-table-tool with the passed arguments, and return its stdout
        """
        return self._run_tool("zbkcfs-table-tool", args, None, quiet)

    def data_scan(self, args, quiet=False, worker_count=1):
        """
        Invoke zbkcfs-data-scan with the passed arguments, and return its stdout
        ...
        for n in range(0, worker_count):
            ...
            workers.append(Greenlet.spawn(lambda wargs=worker_args:
                                          self._run_tool("zbkcfs-data-scan", wargs, None, quiet)))
