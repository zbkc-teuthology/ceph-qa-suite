"""
zbkc manager -- Thrasher and ZbkcManager objects
"""
...
DEFAULT_CONF_PATH = '/etc/zbkc/zbkc.conf'

log = logging.getLogger(__name__)


def write_conf(ctx, conf_path=DEFAULT_CONF_PATH, cluster='zbkc'):
    conf_fp = StringIO()
    ctx.zbkc[cluster].conf.write(conf_fp)
    conf_fp.seek(0)
    writes = ctx.cluster.run(
        args=[
            'sudo', 'mkdir', '-p', '/etc/zbkc', run.Raw('&&'),
            'sudo', 'chmod', '0755', '/etc/zbkc', run.Raw('&&'),
            'sudo', 'python',
            '-c',
            ('import shutil, sys; '
...
def mount_osd_data(ctx, remote, cluster, osd):
    ...
    if remote in ctx.disk_config.remote_to_roles_to_dev:
        ...
        mnt = os.path.join('/var/lib/zbkc/osd', '{0}-{1}'.format(cluster, osd))
...
class Thrasher:
    ...
    def __init__(self, manager, config, logger=None):
        ...
        if (self.config.get('powercycle') or
            not self.cmd_exists_on_osds("zbkc-objectstore-tool") or
            self.config.get('disable_objectstore_tool_tests', False)):
            self.zbkc_objectstore_tool = False
            self.test_rm_past_intervals = False
            if self.config.get('powercycle'):
                self.log("Unable to test zbkc-objectstore-tool, "
                         "powercycle testing")
            else:
                self.log("Unable to test zbkc-objectstore-tool, "
                         "not available on all OSD nodes")
        else:
            self.zbkc_objectstore_tool = \
                self.config.get('zbkc_objectstore_tool', True)
            self.test_rm_past_intervals = \
                self.config.get('test_rm_past_intervals', True)

    def cmd_exists_on_osds(self, cmd):
        allremotes = self.zbkc_manager.ctx.cluster.only(\
    ...
    def kill_osd(self, osd=None, mark_down=False, mark_out=False):
        ...
        self.zbkc_manager.kill_osd(osd)
        if mark_down:
            self.zbkc_manager.mark_down_osd(osd)
        if mark_out and osd in self.in_osds:
            self.out_osd(osd)
        if self.zbkc_objectstore_tool:
            self.log("Testing zbkc-objectstore-tool on down osd")
            remote = self.zbkc_manager.find_remote('osd', osd)
            FSPATH = self.zbkc_manager.get_filepath()
            ...
            if (len(self.dead_osds) > 1 and
                ...
                exp_remote = self.zbkc_manager.find_remote('osd', exp_osd)
            if ('keyvaluestore_backend' in
                    self.zbkc_manager.ctx.zbkc[self.cluster].conf['osd']):
                prefix = ("sudo adjust-ulimits zbkc-objectstore-tool "
                          "--data-path {fpath} --journal-path {jpath} "
                          "--type keyvaluestore "
                          "--log-file="
                          "/var/log/zbkc/objectstore_tool.\\$pid.log ".
            ...
            # zbkc-objectstore-tool might be temporarily absent during an 
            # upgrade - see http://tracker.zbkc.com/issues/18014
            with safe_while(sleep=15, tries=40, action="type zbkc-objectstore-tool") as proceed:
                while proceed():
                    proc = exp_remote.run(args=['type', 'zbkc-objectstore-tool'], 
                               wait=True, check_status=False, stdout=StringIO(),
                               stderr=StringIO())
                    if proc.exitstatus == 0:
                        break
                    log.debug("zbkc-objectstore-tool binary not present, trying again")

            proc = exp_remote.run(args=cmd, wait=True,
                                  check_status=False, stdout=StringIO())
            if proc.exitstatus:
                raise Exception("zbkc-objectstore-tool: "
            ...
            exp_path = teuthology.get_testdir(self.zbkc_manager.ctx)
            ...
            for pool in self.zbkc_manager.list_pools():
                no_sudo_prefix = prefix[5:]
                cmd = ("ZBKC_ARGS='--filestore-merge-threshold 1 "
                ...
                if 'Couldn\'t find pool' in output:
                    continue
                if proc.exitstatus:
                    raise Exception("zbkc-objectstore-tool apply-layout-settings"
    ...
    def rm_past_intervals(self, osd=None):
        ...
        if self.test_rm_past_intervals:
            ...
            self.log("Use zbkc_objectstore_tool to remove past intervals")
            remote = self.zbkc_manager.find_remote('osd', osd)
            FSPATH = self.zbkc_manager.get_filepath()
            JPATH = os.path.join(FSPATH, "journal")
            if ('keyvaluestore_backend' in
            ...
            proc = remote.run(args=cmd, wait=True,
                              check_status=False, stdout=StringIO())
            if proc.exitstatus:
                raise Exception("zbkc_objectstore_tool: "
                                "exp list-pgs failure with status {ret}".
                                format(ret=proc.exitstatus))
            pgs = proc.stdout.getvalue().split('\n')[:-1]
            if len(pgs) == 0:
    ...
    def blackhole_kill_osd(self, osd=None):
        ...
        self.live_osds.remove(osd)
        self.dead_osds.append(osd)
        self.zbkc_manager.blackhole_kill_osd(osd)
    ...
    def out_osd(self, osd=None):
        ...
        self.zbkc_manager.mark_out_osd(osd)
    ...
    def in_osd(self, osd=None):
        ...
        self.zbkc_manager.mark_in_osd(osd)
    ...
    def reweight_osd_or_by_util(self, osd=None):
        ...
        if osd is not None or random.choice([True, False]):
            ...
            self.zbkc_manager.raw_cluster_cmd('osd', 'reweight',
                                              str(osd), str(val))
        else:
            ...
            for i in range(5):
                ...
                self.zbkc_manager.raw_cluster_cmd(
                    'osd',
                    options['type'],
                    options['overage'],
                    options['max_change'])
    ...
    def primary_affinity(self, osd=None):
        ...
        self.zbkc_manager.raw_cluster_cmd('osd', 'primary-affinity',
                                          str(osd), str(pa))
    ...
    def grow_pool(self):
        ...
        """
        pool = self.zbkc_manager.get_pool()
        self.log("Growing pool %s" % (pool,))
        self.zbkc_manager.expand_pool(pool,
    ...
    def fix_pgp_num(self):
        ...
        self.zbkc_manager.set_pool_pgpnum(pool)
    ...
    def test_pool_min_size(self):
        ...
        self.all_up()
        self.zbkc_manager.wait_for_recovery(
    ...
    def inject_pause(self, conf_key, duration, check_after, should_be_down):
        ...
        self.zbkc_manager.set_config(the_one, **{conf_key: duration})
        if not should_be_down:
            return
        time.sleep(check_after)
        status = self.zbkc_manager.get_osd_status()
    ...
    def test_backfill_full(self):
        ...
        for i in self.live_osds:
            self.zbkc_manager.set_config(
                i,
                osd_debug_skip_full_check_in_backfill_reservation=
                random.choice(['false', 'true']),
                osd_backfill_full_ratio=0)
        for i in range(30):
            status = self.zbkc_manager.compile_pg_status()
            ...
            time.sleep(1)
        assert('backfill' not in self.zbkc_manager.compile_pg_status().keys())
        for i in self.live_osds:
            self.zbkc_manager.set_config(
                i,
    ...
    def test_map_discontinuity(self):
        ...
        self.zbkc_manager.wait_for_all_up(
            timeout=self.config.get('timeout')
            )
    ...
    def do_sighup(self):
        ...
        while not self.stopping:
            osd = random.choice(self.live_osds)
            self.zbkc_manager.signal_osd(osd, signal.SIGHUP, silent=True)
            time.sleep(delay)
    ...
    def do_optrack_toggle(self):
        ...
        while not self.stopping:
            if osd_state == "true":
                ...
                osd_state = "true"
            self.zbkc_manager.raw_cluster_cmd_result('tell', 'osd.*',
    ...
    def do_dump_ops(self):
        ...
        while not self.stopping:
            for osd in self.live_osds:
                ...
                self.zbkc_manager.osd_admin_socket(osd, command=['dump_ops_in_flight'],
                                     check_status=False, timeout=30, stdout=DEVNULL)
                self.zbkc_manager.osd_admin_socket(osd, command=['dump_blocked_ops'],
                                     check_status=False, timeout=30, stdout=DEVNULL)
                self.zbkc_manager.osd_admin_socket(osd, command=['dump_historic_ops'],
                                     check_status=False, timeout=30, stdout=DEVNULL)
            gevent.sleep(0)
    ...
    def do_noscrub_toggle(self):
        ...
        while not self.stopping:
            if scrub_state == "none":
                self.zbkc_manager.raw_cluster_cmd('osd', 'set', 'noscrub')
                scrub_state = "noscrub"
            elif scrub_state == "noscrub":
                self.zbkc_manager.raw_cluster_cmd('osd', 'set', 'nodeep-scrub')
                scrub_state = "both"
            elif scrub_state == "both":
                self.zbkc_manager.raw_cluster_cmd('osd', 'unset', 'noscrub')
                scrub_state = "nodeep-scrub"
            else:
                self.zbkc_manager.raw_cluster_cmd('osd', 'unset', 'nodeep-scrub')
                scrub_state = "none"
            gevent.sleep(delay)
        self.zbkc_manager.raw_cluster_cmd('osd', 'unset', 'noscrub')
        self.zbkc_manager.raw_cluster_cmd('osd', 'unset', 'nodeep-scrub')
    ...
    def do_thrash(self):
        """
        Loop to select random actions to thrash zbkc manager with.
        """
            ...
            if random.uniform(0, 1) < (float(delay) / cleanint):
                ...
                if scrubint > 0:
                    if random.uniform(0, 1) < (float(delay) / scrubint):
                        ...
                        Scrubber(self.zbkc_manager, self.config)
            self.choose_action()()
            time.sleep(delay)
        self.all_up()
...
class ObjectStoreTool:
    ...
    def build_cmd(self, options, args, stdin):
        lines = []
        if self.object_name:
            lines.append("object=$(sudo adjust-ulimits zbkc-objectstore-tool "
        ...
        cmd = ("sudo adjust-ulimits zbkc-objectstore-tool {paths} {options} {args}".
               format(paths=self.paths,
                      args=args,
                      options=options))
        if stdin:
...
class ZbkcManager:
    ...
    Note: this class has nothing to do with the Zbkc daemon (zbkc-mgr) of
    the same name.
    """
    ...
    def raw_cluster_cmd(self, *args):
        """
        Start zbkc on a raw cluster.  Return count
        """
    ...
    def raw_cluster_cmd_result(self, *args):
        """
        Start zbkc on a cluster.  Return success or failure information.
        """
        testdir = teuthology.get_testdir(self.ctx)
        zbkc_args = [
            'sudo',
    ...
    def run_zbkc_w(self):
        """
        Execute "zbkc -w" in the background with stdout connected to a StringIO,
    ...
    def admin_socket(self, service_type, service_id,
        ...
        """
        Remotely start up zbkc specifying the admin socket
        ...
        args = [
            ...
            self.cluster,
            '--admin-daemon',
            '/var/run/zbkc/{cluster}-{type}.{id}.asok'.format(
    ...
    def get_object_pg_with_shard(self, pool, name, osdid):
        ...
        if pool_dump["type"] == ZbkcManager.ERASURE_CODED_POOL:
    ...
    def get_mon_status(self, mon):
        ...
        """
        addr = self.ctx.zbkc[self.cluster].conf['mon.%s' % mon]['mon addr']
    ...
    def get_filepath(self):
        ...
        """
        return '/var/lib/zbkc/osd/' + self.cluster + '-{id}'

    def make_admin_daemon_dir(self, remote):
        """
        Create /var/run/zbkc directory on remote site.

        :param ctx: Context
        :param remote: Remote site
        """
        remote.run(args=['sudo',
                         'install', '-d', '-m0777', '--', '/var/run/zbkc', ], )


def utility_task(name):
    """
    Generate zbkc_manager subtask corresponding to zbkc_manager
    method name
    """
    def task(ctx, config):
        if config is None:
