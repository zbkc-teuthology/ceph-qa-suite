...
from zbkcfs.fuse_mount import FuseMount
...
def get_client_configs(ctx, config):
    ...
    teuthology.deep_merge(config, overrides.get('zbkc-fuse', {}))
...
def task(ctx, config):
    """
    Mount/unmount a ``zbkc-fuse`` client.
    ...
    """
    log.info('Mounting zbkc-fuse clients...')
    ...
    for id_, remote in clients:
        ...
        if id_ not in all_mounts:
            ...
            # Catch bad configs where someone has e.g. tried to use zbkc-fuse and kzbkcfs for the same client
    ...
    try:
        ...
        log.info('Unmounting zbkc-fuse clients...')
