"""
zbkc_objectstore_tool - Simple test of zbkc-objectstore-tool utility
"""
...
# Should get cluster name "zbkc" from somewhere
# and normal path from osd_data and osd_journal in conf
FSPATH = "/var/lib/zbkc/osd/zbkc-{id}"
JPATH = "/var/lib/zbkc/osd/zbkc-{id}/journal"
...
def task(ctx, config):
    """
    Run zbkc_objectstore_tool test
    ...
    assert isinstance(config, dict), \
        'zbkc_objectstore_tool task only accepts a dict for configuration'

    log.info('Beginning zbkc_objectstore_tool...')
    ...
    try:
        ...
        log.info('Ending zbkc_objectstore_tool')
...
def test_objectstore(ctx, config, cli_remote, REP_POOL, REP_NAME, ec=False):
    ...
    DATADIR = os.path.join(TEUTHDIR, "zbkc.data")
    DATALINECOUNT = 10000
    ERRORS = 0
    ...
    for stats in manager.get_pg_stats():
        if stats["pgid"].find(str(REPID) + ".") != 0:
            continue
        if pool_dump["type"] == zbkc_manager.ZbkcManager.REPLICATED_POOL:
            for osd in stats["acting"]:
                pgs.setdefault(osd, []).append(stats["pgid"])
        elif pool_dump["type"] == zbkc_manager.ZbkcManager.ERASURE_CODED_POOL:
            shard = 0
            for osd in stats["acting"]:
    ...
    prefix = ("sudo zbkc-objectstore-tool "
              "--data-path {fpath} "
