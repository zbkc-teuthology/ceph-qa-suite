...
Handle the setup, starting, and clean-up of a Zbkc cluster.
"""
...
import zbkc_client as cclient
from teuthology.orchestra.daemon import DaemonGroup

ZBKC_ROLE_TYPES = ['mon', 'mgr', 'osd', 'mds', 'rgw']
...
def zbkc_log(ctx, config):
    """
    Create /var/log/zbkc log directory that is open to everyone.
    Add valgrind and profiling-logger directories.

    :param ctx: Context
    :param config: Configuration
    """
    log.info('Making zbkc log dir writeable by non-root...')
    run.wait(
        ctx.cluster.run(
            args=[
                'sudo',
    ...
    log.info('Disabling zbkc logrotate...')
    run.wait(
        ctx.cluster.run(
            args=[
                'sudo',
                'rm', '-f', '--',
                '/etc/logrotate.d/zbkc',
            ],
            wait=False,
        )
    )
    ...
    run.wait(
        ctx.cluster.run(
            args=[
                'sudo',
                'install', '-d', '-m0777', '--',
                '/var/log/zbkc/valgrind',
                '/var/log/zbkc/profiling-logger',
            ],
            wait=False,
        )
    )
    ...
    class Rotater(object):
        ...
        def invoke_logrotate(self):
            # 1) install zbkc-test.conf in /etc/logrotate.d
            # 2) continuously loop over logrotate invocation with zbkc-test.conf
            while not self.stop_event.is_set():
                self.stop_event.wait(timeout=30)
                try:
                    run.wait(
                        ctx.cluster.run(
                            args=['sudo', 'logrotate', '/etc/logrotate.d/zbkc-test.conf'
                                  ],
                            wait=False,
                        )
                    )
        ...
        with file(rotate_conf_path, 'rb') as f:
            ...
            for remote in ctx.cluster.remotes.iterkeys():
                teuthology.write_file(remote=remote,
                                      path='{tdir}/logrotate.zbkc-test.conf'.format(tdir=testdir),
                                      data=StringIO(conf)
                                      )
                remote.run(
                    args=[
                        'sudo',
                        'mv',
                        '{tdir}/logrotate.zbkc-test.conf'.format(tdir=testdir),
                        '/etc/logrotate.d/zbkc-test.conf',
                        run.Raw('&&'),
                        'sudo',
                        'chmod',
                        '0644',
                ...
                remote.chcon('/etc/logrotate.d/zbkc-test.conf',
    ...
    try:
        ...
        if ctx.config.get('log-rotate'):
            ...
            logrotater.end()
            ctx.cluster.run(
                args=['sudo', 'rm', '/etc/logrotate.d/zbkc-test.conf'
                      ]
            )
...
def crush_setup(ctx, config):
    ...
    mon_remote.run(
        args=['sudo', 'zbkc', '--cluster', cluster_name,
              'osd', 'crush', 'tunables', profile])
    yield


@contextlib.contextmanager
def zbkcfs_setup(ctx, config):
    cluster_name = config['cluster']
    testdir = teuthology.get_testdir(ctx)
    ...
    if mdss.remotes:
        log.info('Setting up ZbkcFS filesystem...')

        Filesystem(ctx, create='zbkcfs') # TODO: make Filesystem cluster-aware
        ...
        mon_remote.run(
            args=[
                ...
                'zbkc-coverage',
                coverage_dir,
                'zbkc', 'mds', 'set', 'allow_multimds', 'true',
        ...
        mon_remote.run(
            args=[
                ...
                'zbkc-coverage',
                coverage_dir,
                'zbkc', 'mds', 'set', 'allow_dirfrags', 'true',
                '--yes-i-really-mean-it'],
	    check_status=False,  # probably old version, upgrade test
        )
...
def cluster(ctx, config):
    """
    Handle the creation and removal of a zbkc cluster.
    ...
    On exit:
        ...
        Save the monitor information and archive all zbkc logs.
    ...
    log.info('Creating zbkc cluster %s...', cluster_name)
    run.wait(
        ctx.cluster.run(
            args=[
                'install', '-d', '-m0755', '--',
    ...
    for remote, roles_to_journals in remote_to_roles_to_journals.iteritems():
        for role, journal in roles_to_journals.iteritems():
            name = teuthology.zbkc_role(role)
            if name not in conf:
                conf[name] = {}
    ...
    if not hasattr(ctx, 'zbkc'):
        ctx.zbkc = {}
    ctx.zbkc[cluster_name] = argparse.Namespace()
    ctx.zbkc[cluster_name].conf = conf

    default_keyring = '/etc/zbkc/{cluster}.keyring'.format(cluster=cluster_name)
    ...
    default_conf_path = '/etc/zbkc/{cluster}.conf'.format(cluster=cluster_name)
    ...
    for remote, roles_for_host in mgrs.remotes.iteritems():
        for role in teuthology.cluster_roles_of_type(roles_for_host, 'mgr',
            ...
            mgr_dir = '/var/lib/zbkc/mgr/{cluster}-{id}'.format(
                cluster=cluster_name,
                id=id_,
            )
            remote.run(
    ...
    for remote, roles_for_host in mdss.remotes.iteritems():
        for role in teuthology.cluster_roles_of_type(roles_for_host, 'mds',
                                                     cluster_name):
            _, _, id_ = teuthology.split_role(role)
            mds_dir = '/var/lib/zbkc/mds/{cluster}-{id}'.format(
                cluster=cluster_name,
                id=id_,
            )
            remote.run(
    ...
    for remote, roles_for_host in osds.remotes.iteritems():
        ...
        for role in teuthology.cluster_roles_of_type(roles_for_host, 'osd', cluster_name):
            _, _, id_ = teuthology.split_role(role)
            mnt_point = '/var/lib/zbkc/osd/{cluster}-{id}'.format(cluster=cluster_name, id=id_)
            remote.run(
                args=[
                    'sudo',
                    'mkdir',
        ...
        for type_ in ['mgr',  'mds', 'osd']:
            for role in teuthology.cluster_roles_of_type(roles_for_host, type_, cluster_name):
                ...
                data = teuthology.get_file(
                    remote=remote,
                    path='/var/lib/zbkc/{type}/{cluster}-{id}/keyring'.format(
                        type=type_,
                        id=id_,
                        cluster=cluster_name,
                    ),
    ...
    for remote, roles_for_host in ctx.cluster.remotes.iteritems():
        for role in teuthology.cluster_roles_of_type(roles_for_host, 'client', cluster_name):
            ...
            data = teuthology.get_file(
                remote=remote,
                path='/etc/zbkc/{cluster}.client.{id}.keyring'.format(id=id_, cluster=cluster_name)
            )
        ...
        for role in teuthology.cluster_roles_of_type(roles_for_host, 'mon', cluster_name):
            ...
            remote.run(
                args=[
                    ...
                    'mkdir',
                    '-p',
                    '/var/lib/zbkc/mon/{cluster}-{id}'.format(id=id_, cluster=cluster_name),
                ],
            )
            remote.run(
                args=[
    ...
    try:
        ...
        def first_in_zbkc_log(pattern, excludes):
            """
            Find the first occurence of the pattern specified in the Zbkc log,
            ...
            args = [
                'sudo',
                'egrep', pattern,
                '/var/log/zbkc/{cluster}.log'.format(cluster=cluster_name),
            ]
        ...
        if first_in_zbkc_log('\[ERR\]|\[WRN\]|\[SEC\]',
            ...
            if 'failure_reason' not in ctx.summary:
                for pattern in ['\[SEC\]', '\[ERR\]', '\[WRN\]']:
                    match = first_in_zbkc_log(pattern, config['log_whitelist'])
        ...
        log.info('Cleaning zbkc cluster...')
        run.wait(
            ctx.cluster.run(
                args=[
                    'sudo',
    ...
    for remote, roles_for_host in daemons.remotes.iteritems():
        ...
        for role in roles_for_host:
            ...
            if type_ in config.get('cpu_profile', []):
                profile_path = '/var/log/zbkc/profiling-logger/%s.prof' % (role)
...
def healthy(ctx, config):
    """
    Wait for all osd's to be up, and for the zbkc health monitor to return HEALTH_OK.

    :param ctx: Context
    :param config: Configuration
    """
    config = config if isinstance(config, dict) else dict()
    cluster_name = config.get('cluster', 'zbkc')
    log.info('Waiting until zbkc cluster %s is healthy...', cluster_name)
    firstmon = teuthology.get_first_mon(ctx, config, cluster_name)
    (mon0_remote,) = ctx.cluster.only(firstmon).remotes.keys()
    ...
    if ctx.cluster.only(teuthology.is_type('mds', cluster_name)).remotes:
        ...
        zbkc_fs = Filesystem(ctx) # TODO: make Filesystem cluster-aware
        zbkc_fs.wait_for_daemons(timeout=300)
...
def wait_for_osds_up(ctx, config):
    ...
    :param config: Configuration
    """
    log.info('Waiting until zbkc osds are all up...')
    cluster_name = config.get('cluster', 'zbkc')
    firstmon = teuthology.get_first_mon(ctx, config, cluster_name)
    (mon0_remote,) = ctx.cluster.only(firstmon).remotes.keys()
    teuthology.wait_until_osds_up(
...
def wait_for_mon_quorum(ctx, config):
    """
    Check renote zbkc status until all monitors are up.
...
def created_pool(ctx, config):
    """
    Add new pools to the dictionary of pools that the zbkc-manager
    knows about.
    """
    for new_pool in config:
        if new_pool not in ctx.managers['zbkc'].pools:
            ctx.managers['zbkc'].pools[new_pool] = ctx.managers['zbkc'].get_pool_property(
                new_pool, 'pg_num')
...
def restart(ctx, config):
   ...
   or::
    ...
    daemons = ctx.daemons.resolve_role_list(config.get('daemons', None), ZBKC_ROLE_TYPES, True)
    clusters = set()
    for role in daemons:
...
def wait_for_failure(ctx, config):
    """
    Wait for a failure of a zbkc daemon

    For example::
      tasks:
      - zbkc.wait_for_failure: [mds.*]

      tasks:
      - zbkc.wait_for_failure: [osd.0, osd.2]

      tasks:
      - zbkc.wait_for_failure:
          daemons: [osd.0, osd.2]
...
def task(ctx, config):
    ...
    To run multiple zbkc clusters, use multiple zbkc tasks, and roles
    with a cluster name prefix, e.g. cluster1.client.0. Roles with no
    cluster use the default cluster name, 'zbkc'. OSDs from separate
    ...
    instance of the zbkc task with the cluster name specified, e.g.::
    ...
    teuthology.deep_merge(config, overrides.get('zbkc', {}))

    first_zbkc_cluster = False
    if not hasattr(ctx, 'daemons'):
        first_zbkc_cluster = True
    ...
    subtasks += [
        ...
        lambda: zbkcfs_setup(ctx=ctx, config=config),
    ...
    with contextutil.nested(*subtasks):
        ...
        if not hasattr(ctx, 'managers'):
            ctx.managers = {}
        ctx.managers[config['cluster']] = ZbkcManager(
            mon,
            ctx=ctx,
            logger=log.getChild('zbkc_manager.' + config['cluster']),
            cluster=config['cluster'],
        )
