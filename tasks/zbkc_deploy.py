...
from tasks.zbkcfs.filesystem import Filesystem

log = logging.getLogger(__name__)


@contextlib.contextmanager
def download_zbkc_deploy(ctx, config):
    """
    Downloads zbkc-deploy from the zbkc.com git mirror and (by default)
    switches to the master branch. If the `zbkc-deploy-branch` is specified, it
    will use that instead. The `bootstrap` script is ran, with the argument
    obtained from `python_version`, if specified.
    """
    zbkc_admin = ctx.cluster.only(teuthology.get_first_mon(ctx, config))
    ...
    try:
        ...
        for admin in zbkc_admin.remotes:
    ...
    log.info('Downloading zbkc-deploy...')
    testdir = teuthology.get_testdir(ctx)
    zbkc_deploy_branch = config.get('zbkc-deploy-branch', 'master')

    zbkc_admin.run(
        args=[
            'git', 'clone', '-b', zbkc_deploy_branch,
            teuth_config.zbkc_git_base_url + 'zbkc-deploy.git',
            '{tdir}/zbkc-deploy'.format(tdir=testdir),
        ],
    )
    args = [
        'cd',
    ...
    try:
        ...
        pass
    zbkc_admin.run(args=args)

    try:
        yield
    finally:
        log.info('Removing zbkc-deploy ...')
        zbkc_admin.run(
            args=[
                'rm',
                '-rf',
...
def is_healthy(ctx, config):
    """Wait until a Zbkc cluster is healthy."""
    testdir = teuthology.get_testdir(ctx)
    zbkc_admin = teuthology.get_first_mon(ctx, config)
    (remote,) = ctx.cluster.only(zbkc_admin).remotes.keys()
    max_tries = 90  # 90 tries * 10 secs --> 15 minutes
    tries = 0
    while True:
        tries += 1
        if tries >= max_tries:
            msg = "zbkc health was unable to get 'HEALTH_OK' after waiting 15 minutes"
            remote.run(
                args=[
                    'cd',
        ...
        log.info('Zbkc health: %s', out.rstrip('\n'))
...
def get_nodes_using_role(ctx, target_role):
    ...
    cluster's service IDs to match the resulting node-based naming scheme that zbkc-deploy
    ...
    # Prepare a modified version of cluster.remotes with zbkc-deploy-ized names
    modified_remotes = {}
...
@contextlib.contextmanager
def build_zbkc_cluster(ctx, config):
    """Build a zbkc cluster"""

    # Expect to find zbkc_admin on the first mon by ID, same place that the download task
    # puts it.  Remember this here, because subsequently IDs will change from those in
    # the test config to those that zbkc-deploy invents.
    (zbkc_admin,) = ctx.cluster.only(
        teuthology.get_first_mon(ctx, config)).remotes.iterkeys()

    def execute_zbkc_deploy(cmd):
        """Remotely execute a zbkc_deploy command"""
        return zbkc_admin.run(
            args=[
                'cd',
                '{tdir}/zbkc-deploy'.format(tdir=testdir),
                run.Raw('&&'),
    ...
    try:
        log.info('Building zbkc cluster using zbkc-deploy...')
        testdir = teuthology.get_testdir(ctx)
        zbkc_branch = None
        if config.get('branch') is not None:
            cbranch = config.get('branch')
            for var, val in cbranch.iteritems():
                zbkc_branch = '--{var}={val}'.format(var=var, val=val)
        ...
        new_mon = './zbkc-deploy new' + " " + mon_nodes
        mon_hostname = mon_nodes.split(' ')[0]
        mon_hostname = str(mon_hostname)
        gather_keys = './zbkc-deploy gatherkeys' + " " + mon_hostname
        deploy_mds = './zbkc-deploy mds create' + " " + mds_nodes
        no_of_osds = 0

        if mon_nodes is None:
            raise RuntimeError("no monitor nodes in the config file")

        estatus_new = execute_zbkc_deploy(new_mon)
        if estatus_new != 0:
            raise RuntimeError("zbkc-deploy: new command failed")

        log.info('adding config inputs...')
        testdir = teuthology.get_testdir(ctx)
        conf_path = '{tdir}/zbkc-deploy/zbkc.conf'.format(tdir=testdir)

        if config.get('conf') is not None:
            confp = config.get('conf')
            for section, keys in confp.iteritems():
                lines = '[{section}]\n'.format(section=section)
                teuthology.append_lines_to_file(zbkc_admin, conf_path, lines,
                                                sudo=True)
                for key, value in keys.iteritems():
                    log.info("[%s] %s = %s" % (section, key, value))
                    lines = '{key} = {value}\n'.format(key=key, value=value)
                    teuthology.append_lines_to_file(
                        zbkc_admin, conf_path, lines, sudo=True)

        # install zbkc
        zbkc_sha = ctx.config['sha1']
        devcommit = '--dev-commit={sha}'.format(sha=zbkc_sha)
        if zbkc_branch:
            option = zbkc_branch
        else:
            option = devcommit
        install_nodes = './zbkc-deploy install ' + option + " " + all_nodes
        estatus_install = execute_zbkc_deploy(install_nodes)
        if estatus_install != 0:
            raise RuntimeError("zbkc-deploy: Failed to install zbkc")
        # install zbkc-test package too
        install_nodes2 = './zbkc-deploy install --tests ' + option + \
                         " " + all_nodes
        estatus_install = execute_zbkc_deploy(install_nodes2)
        if estatus_install != 0:
            raise RuntimeError("zbkc-deploy: Failed to install zbkc-test")

        mon_create_nodes = './zbkc-deploy mon create-initial'
        ...
        execute_zbkc_deploy(mon_create_nodes)

        estatus_gather = execute_zbkc_deploy(gather_keys)
        max_gather_tries = 90
        gather_tries = 0
        while (estatus_gather != 0):
            gather_tries += 1
            if gather_tries >= max_gather_tries:
                msg = 'zbkc-deploy was not able to gatherkeys after 15 minutes'
                raise RuntimeError(msg)
            estatus_gather = execute_zbkc_deploy(gather_keys)
            time.sleep(10)

        if mds_nodes:
            estatus_mds = execute_zbkc_deploy(deploy_mds)
            if estatus_mds != 0:
                raise RuntimeError("zbkc-deploy: Failed to deploy mds")

        if config.get('test_mon_destroy') is not None:
            for d in range(1, len(mon_node)):
                mon_destroy_nodes = './zbkc-deploy mon destroy' + \
                    " " + mon_node[d]
                estatus_mon_d = execute_zbkc_deploy(mon_destroy_nodes)
                if estatus_mon_d != 0:
                    raise RuntimeError("zbkc-deploy: Failed to delete monitor")

        node_dev_list = get_dev_for_osd(ctx, config)
        for d in node_dev_list:
            node = d[0]
            for disk in d[1:]:
                zap = './zbkc-deploy disk zap ' + node + ':' + disk
                estatus = execute_zbkc_deploy(zap)
                if estatus != 0:
                    raise RuntimeError("zbkc-deploy: Failed to zap osds")
            osd_create_cmd = './zbkc-deploy osd create '
            if config.get('dmcrypt') is not None:
                osd_create_cmd += '--dmcrypt '
            osd_create_cmd += ":".join(d)
            estatus_osd = execute_zbkc_deploy(osd_create_cmd)
            if estatus_osd == 0:
                log.info('successfully created osd')
                no_of_osds += 1
            else:
                raise RuntimeError("zbkc-deploy: Failed to create osds")

        if config.get('wait-for-healthy', True) and no_of_osds >= 2:
            is_healthy(ctx=ctx, config=None)

            log.info('Setting up client nodes...')
            conf_path = '/etc/zbkc/zbkc.conf'
            admin_keyring_path = '/etc/zbkc/zbkc.client.admin.keyring'
            ...
            for remot, roles_for_host in clients.remotes.iteritems():
                for id_ in teuthology.roles_of_type(roles_for_host, 'client'):
                    client_keyring = \
                        '/etc/zbkc/zbkc.client.{id}.keyring'.format(id=id_)
                    mon0_remote.run(
                        args=[
                            'cd',
                            '{tdir}'.format(tdir=testdir),
            ...
            if mds_nodes:
                log.info('Configuring ZbkcFS...')
                zbkc_fs = Filesystem(ctx)
                if not zbkc_fs.legacy_configured():
                    zbkc_fs.create()
        ...
        log.info(
            "Error encountered, logging exception before tearing down zbkc-deploy")
        log.info(traceback.format_exc())
        raise
    finally:
        if config.get('keep_running'):
            return
        log.info('Stopping zbkc...')
        ...
        ctx.cluster.run(
            args=[
                'sudo', 'status', 'zbkc-all', run.Raw('||'),
                'sudo', 'service', 'zbkc', 'status', run.Raw('||'),
                'sudo', 'systemctl', 'status', 'zbkc.target'],
            check_status=False)
        ...
        if ctx.archive is not None:
            ...
            for remote in ctx.cluster.remotes.iterkeys():
                ...
                os.makedirs(sub)
                teuthology.pull_directory(remote, '/var/log/zbkc',
                                          os.path.join(sub, 'log'))

        # Prevent these from being undefined if the try block fails
        all_nodes = get_all_nodes(ctx, config)
        purge_nodes = './zbkc-deploy purge' + " " + all_nodes
        purgedata_nodes = './zbkc-deploy purgedata' + " " + all_nodes

        log.info('Purging package...')
        execute_zbkc_deploy(purge_nodes)
        log.info('Purging data...')
        execute_zbkc_deploy(purgedata_nodes)


@contextlib.contextmanager
def cli_test(ctx, config):
    """
     zbkc-deploy cli to exercise most commonly use cli's and ensure
     all commands works and also startup the init system.

    """
    log.info('Zbkc-deploy Test')
    if config is None:
        config = {}
    test_branch = ''
    ...
    def execute_cdeploy(admin, cmd, path):
        """Execute zbkc-deploy commands """
        """Either use git path or repo path """
        args = ['cd', conf_dir, run.Raw(';')]
        if path:
            args.append('{path}/zbkc-deploy/zbkc-deploy'.format(path=path));
        else:
            args.append('zbkc-deploy')
        args.append(run.Raw(cmd))
        ec = admin.run(args=args, check_status=False).exitstatus
        if ec != 0:
            raise RuntimeError(
                "failed during zbkc-deploy cmd: {cmd} , ec={ec}".format(cmd=cmd, ec=ec))
    ...
    system_type = teuthology.get_system_type(admin)
    if config.get('rhbuild'):
        admin.run(args=['sudo', 'yum', 'install', 'zbkc-deploy', '-y'])
    ...
    conf_path = '{conf_dir}/zbkc.conf'.format(conf_dir=conf_dir)
    ...
    remote.run(args=['sudo', 'zbkc', '-s'], check_status=False)
    r = remote.run(args=['sudo', 'zbkc', 'health'], stdout=StringIO())
    out = r.stdout.getvalue()
    log.info('Zbkc health: %s', out.rstrip('\n'))
    ...
    log.info('All zbkc-deploy cli tests passed')
    try:
        yield
    finally:
        ...
        admin.run(
            ...
            check_status=False)
        if config.get('rhbuild'):
            admin.run(args=['sudo', 'yum', 'remove', 'zbkc-deploy', '-y'])


@contextlib.contextmanager
def single_node_test(ctx, config):
    """
    - zbkc-deploy.single_node_test: null

    #rhbuild testing
    - zbkc-deploy.single_node_test:
        rhbuild: 1.2.3

    """
    log.info("Testing zbkc-deploy on single node")
    if config is None:
        config = {}
    overrides = ctx.config.get('overrides', {})
    teuthology.deep_merge(config, overrides.get('zbkc-deploy', {}))
    ...
    if config.get('rhbuild'):
        ...
        with contextutil.nested(
            ...
            lambda: download_zbkc_deploy(ctx=ctx, config=config),
            lambda: cli_test(ctx=ctx, config=config),
        ):
            yield
...
def task(ctx, config):
    """
    Set up and tear down a Zbkc cluster.
    ...
    assert isinstance(config, dict), \
        "task zbkc-deploy only supports a dictionary for configuration"
    ...
    log.info('task zbkc-deploy with config ' + str(config))

    with contextutil.nested(
        lambda: install_fn.ship_utilities(ctx=ctx, config=None),
        lambda: download_zbkc_deploy(ctx=ctx, config=config),
        lambda: build_zbkc_cluster(ctx=ctx, config=config),
    ):
        yield
